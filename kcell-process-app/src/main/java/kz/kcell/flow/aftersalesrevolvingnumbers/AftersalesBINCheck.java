...
@Service("AftersalesBINCheck")
@RequestMapping("/aftersalesrevolvingnumbers")
@Log
public class AftersalesBINCheck {
    ...
    @Autowired
    public AftersalesBINCheck(@Value("${mail.message.baseurl:http://localhost}") String baseURL) {
    ...
    public ResponseEntity<String> getPBXDataByBIN(@PathVariable("pbxBIN") String pbxBIN, HttpServletRequest request) throws Exception {
        ...
        List<HistoricProcessInstance> processesRevol = historyService.createHistoricProcessInstanceQuery()
            .processDefinitionKey("revolvingNumbers")
            .variableValueEquals("clientBIN", pbxBIN)
            .finished()
            .orderByProcessInstanceEndTime().desc().list();

        processes.addAll(processesRevol);
        ...
        if (process != null && process.getId() != null) {
            ...
            processesRevol = historyService.createHistoricProcessInstanceQuery()
                .processDefinitionKey("revolvingNumbers")
                .variableValueEquals("clientBIN", pbxBIN)
                .finished()
                .orderByProcessInstanceEndTime().desc().list();

            val additionalProcessesRevol = historyService.createHistoricProcessInstanceQuery()
                .processDefinitionKey("revolvingNumbers")
                .variableValueEquals("clientBIN", pbxBIN)
                .unfinished()
                .orderByProcessInstanceEndTime().desc().list();
            ...
            processes.addAll(processesRevol);
            processes.addAll(additionalProcessesRevol);
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("No process with such BIN found!");
    }
}
