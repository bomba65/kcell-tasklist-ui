...
public class ReportController {
    ...
    @RequestMapping(value = "/financialreportvostok2023", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEntity<List<FinancialReportDto>> getFinancialReportVostok2023(){
        if (identityService.getCurrentAuthentication() == null || identityService.getCurrentAuthentication().getUserId() == null) {
            log.warning("No user logged in");
            return ResponseEntity.ok(new ArrayList<FinancialReportDto>());
        }
        InputStream fis = ReportController.class.getResourceAsStream("/reports/financial-report_contract-vostok-2023.sql");
        Scanner s = new Scanner(fis).useDelimiter("\\A");
        String query = s.hasNext() ? s.next() : "";
        List<FinancialReportDto> reportDtos = reportRepository.financialReport(query);
        return ResponseEntity.ok(reportDtos);
    ...
    @RequestMapping(value = "/extended-report-by-jobs-vostok-2023", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEntity<List<ExtendedReportByJobsDto>> getExtendedReportByJobsVostok2023(){
        if (identityService.getCurrentAuthentication() == null || identityService.getCurrentAuthentication().getUserId() == null) {
            log.warning("No user logged in");
            return ResponseEntity.ok(new ArrayList<ExtendedReportByJobsDto>());
        }
