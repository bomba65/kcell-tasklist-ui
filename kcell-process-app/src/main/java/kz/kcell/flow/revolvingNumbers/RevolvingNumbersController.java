...
@Log
@RestController
@RequestMapping("/revolving-numbers")
public class RevolvingNumbersController {
    ...
    public void technicalCondition(@PathVariable("taskId") String taskId, HttpServletResponse response) throws Exception {
        ...
        XWPFDocument doc = new XWPFDocument(RevolvingNumbersController.class.getResourceAsStream("/revolvingNumbers/technical_conditions_template_rev_num.docx"));
        ...
        String searchCallerID = variables.get("searchCallerID").toString();
        ...
        replaceText(doc, "ipAddress", Arrays.asList((sip.getString("voiceIP") + ", " + sip.getString("signalingIP")).replaceAll(" ", "").split(",")).stream().collect(Collectors.toSet()).stream().collect(Collectors.joining(", ")));
        ...
        BufferedImage image = ImageIO.read(RevolvingNumbersController.class.getResourceAsStream("/PBX/tcSchema.png"));

        Graphics g = image.getGraphics();
        g.setFont(g.getFont().deriveFont(22f));
        Rectangle bounds = new Rectangle(460, 0, 320, 230);
        TextRenderer.drawString(g,
            sip.getString("connectionPoint") + "\n" +
                "Kcell\n" +
                "г. Алматы",
