    ...
    return module.controller('mainCtrl', ['$scope', '$rootScope', 'toasty', 'AuthenticationService', '$routeParams', '$timeout', '$location', 'exModal', '$http', '$state', 'StartProcessService', function ($scope, $rootScope, toasty, AuthenticationService, $routeParams, $timeout, $location, exModal, $http, $state, StartProcessService) {
        ...
        $scope.getAdditionalVariablesToTasks = function (variableList) {
            _.forEach(variableList, function (variable) {
                var varSearchParams = {
                    processInstanceIdIn: _.map($scope.currentTaskGroup.tasks, 'processInstanceId'),
            ...
            await $http({
                ...
                if ($scope.processSearchResults.length > 0) {
                    _.forEach(['site_name', 'priority', 'validityDate', 'requestedDate'], function (variable) {
                        var varSearchParams = {
                            processInstanceIdIn: _.map($scope.processSearchResults, 'id'),
                            variableName: variable
                        };
                        $http({
                            method: 'POST',
        ...
        $scope.searchContractorInvoices = function () {
            ...
            if ($scope.invoiceContractorFilter.workType) {
                queryParams.variables.push({
                    name: "workType",
                    value: $scope.invoiceContractorFilter.workType + '',
                    operator: "eq"
                });
            }
            ...
            if ($rootScope.hasGroup('hq_contractor_lse') && $scope.invoiceContractorFilter.region !== 'all') {
                queryParams.variables.push({
                    "name": "siteRegion",
                    "operator": "eq",
                    "value": $scope.invoiceContractorFilter.region
                });
            } else {
            ...
            $http({
                ...
                if ($scope.contractorsSearchResults.length > 0) {
                    _.forEach(['yearOfFormalPeriod', 'workType', 'monthActNumber'], function (variable) {
                        var varSearchParams = {
                            processInstanceIdIn: _.map($scope.contractorsSearchResults, 'id'),
                            variableName: variable
                        };
                        $http({
                            method: 'POST',
        ...
        $scope.toggleProcessView = function (index, id, processDefinitionKey) {
            ...
            if ($scope.piIndex === index) {
                ...
                $scope.piIndex = index;
                $scope.jobModel = {
                    state: $scope.contractorsSearchResults[index].state,
        ...
        function loadTasks() {
            ...
            if ($scope.currentFilter) {
                ...
                var queryData = {
                    ...
                    'processDefinitionKeyIn': processDefinitionKeyMap
                };
                            ...
                            if ($scope.tasks && $scope.tasks.length > 0) {
                                $scope.tasks.forEach(function (e) {
                                    ...
                                    if (!$scope.taskGroups[e.name]) {
                                        $scope.taskGroups[e.name] = {
                                            tasks: [],
                                            taskDefinitionKey: e.taskDefinitionKey,
                                            name: e.name
                                        };
                                    }
    ...
    */
        ...
        $scope.$watchGroup(['selectedProject', 'selectedProcess'], function (newValues, oldValues, scope) {
            if ((newValues[0].key !== oldValues[0].key || newValues[1].key !== oldValues[1].key)) {
                if ($scope.processDefinitionKey && !_.some($rootScope.getCurrentProcesses(), function (pd) {
                    return pd.key === $scope.processDefinitionKey
                })) {
                    $state.go('tasks');
                }
            }
            ...
            $http({
                ...
                function (result) {
                    ...
                    if (result.data._embedded.identityLink && result.data._embedded.identityLink.length > 0) {
                        result.data.candidateObject = result.data._embedded.identityLink.find(function (el) {
            ...
            $http.post('/camunda/api/engine/engine/default/history/variable-instance?deserializeValues=false', variableQuery).then(
                function (result) {
                    if (result.data && result.data[0] && result.data[0].value) {
                        $scope.resolution = JSON.parse(result.data[0].value).find(function (el) {
                            return el.taskId === $stateParams.id;
                        });
...
//					if(e.name != 'null'){
    ...
    }]).controller('catalogsCtrl', ['$scope', '$rootScope', '$http', '$timeout', 'exModal', 'toasty', 'AuthenticationService',
        function ($scope, $rootScope, $http, $timeout, exModal, toasty, AuthenticationService) {

            $scope.selectedProcessKey = $rootScope.selectedProcess.key;
            $scope.searchText = undefined;
            $scope.catalogsNames = [];

            function fetchCatalogList(select) {
                $http.get('/camunda/catalogs/api/getnames/' + $rootScope.selectedProcess.key).then(function (result) {
                    if (result.data && result.data.length) {
                        $scope.catalogsNames = result.data;
                        if (select) {
                            var fetchCatalog = _.find($scope.catalogsNames, function (cn) {
                                return cn.id === select.id;
                            });
                            $scope.selectCatalog(fetchCatalog);
                        } else {
                            $scope.selectCatalog($scope.catalogsNames[0]);
                        }
                    }
                });
            }
            ...
            $scope.newCatalog = function () {
                exModal.open({
                    scope: {
                        newCatalogInfo: {},
                        getCatalogGroupList: function (val) {
                            var users = $http.get('/camunda/api/engine/engine/default/group?nameLike=' + encodeURIComponent('%' + val + '%')).then(
                                function (response) {
                                    return response.data;
                                }
                            );
                        ...
                        selectReaderGroup: function (val) {
                            scope.newCatalogInfo.reader = val;
                        }
                    },
                    templateUrl: './js/partials/catalogs/newCatalogModal.html',
                    size: 'lg'
                }).then(function (newCatalogInfo) {
                    if (!newCatalogInfo.name || "" === newCatalogInfo.name.trim) {
                        toasty.error({title: "Error", msg: 'Provide the name of the catalog'});
                        return;
                    }
                    newCatalogInfo.name = newCatalogInfo.name.trim();
                    if ($scope.catalogsNames.includes(newCatalogInfo.name)) {
                        toasty.error({title: "Error", msg: 'Catalog with such name already exists'});
                        return;
                    }
                    $scope.save(newCatalogInfo);
                }, function () {
                });
            };
                ...
                exModal.open({
                    scope: {
                        newCatalogInfo: {
                            id: catalog.id,
                            name: catalog.name,
                            reader: catalog.readerGroupIds[0],
                            readerName: catalog.readerName,
                            writer: catalog.writerGroupIds[0],
                            writerName: catalog.writerName
                        },
                        getCatalogGroupList: function (val) {
                            var users = $http.get('/camunda/api/engine/engine/default/group?nameLike=' + encodeURIComponent('%' + val + '%')).then(
                                function (response) {
                    ...
                    templateUrl: './js/partials/catalogs/newCatalogModal.html',
                    size: 'lg'
                }).then(function (editedCatalogInfo) {
                    if (!editedCatalogInfo.name || "" === editedCatalogInfo.name.trim) {
                        toasty.error({title: "Error", msg: 'Provide the name of the catalog'});
                        return;
                    }
                    editedCatalogInfo.name = editedCatalogInfo.name.trim();
                    if ($scope.catalogsNames.includes(editedCatalogInfo.name)) {
                        toasty.error({title: "Error", msg: 'Catalog with such name already exists'});
                        return;
                    }
                    $scope.save(editedCatalogInfo);
                });
            };


            $scope.selectCatalog = function (catalog) {
                if (this.currentCatalog && this.currentCatalog.id === catalog.id) {
                    return;
                }
            ...
            function fetchData() {
                ...
                $http.get('/camunda/catalogs/api/get/' + $rootScope.selectedProcess.key + '/' + $scope.currentCatalog.id).then(function (result) {
                    if (result && result.data) {
                        $scope.currentCatalog = result.data;
                        $scope.currentCatalog.createDate = new Date(result.data.createdAt);
                        $scope.currentCatalog.modifyDate = new Date(result.data.modifiedAt);
                        $scope.currentCatalog.data = JSON.parse(JSON.stringify(result.data.data));
                        if ($scope.currentCatalog.userId) {
                            $http.get("/camunda/api/engine/engine/default/user/" + $scope.currentCatalog.userId + "/profile").then(function (result) {
                                $scope.currentCatalog.userId = (result.data.firstName ? result.data.firstName : "") + " " + (result.data.lastName ? result.data.lastName : "");
                            });
                        }
                        $http.get("/camunda/api/engine/engine/default/group/" + $scope.currentCatalog.readerGroupIds[0]).then(function (rr) {
                            $scope.currentCatalog.readerName = rr.data.name;
                        });
                        $http.get("/camunda/api/engine/engine/default/group/" + $scope.currentCatalog.writerGroupIds[0]).then(function (wr) {
                            $scope.currentCatalog.writerName = wr.data.name;
                        });
                        $scope.isLoading = false;
                    }
                }).catch(function (error) {
                    toasty.error({title: "Error", msg: error});
                });
            }

            $scope.addNode = function () {
                $scope.currentCatalog.data.$list.push({id: undefined, value: '', isEditing: true});
            };

            $scope.setEditing = function (index) {
                $scope.currentCatalog.data.$list[index].isEditing = true;
                $scope.currentCatalog.data.$list[index].originalValue = $scope.currentCatalog.data.$list[index].value;
            }

            $scope.cancelEdit = function (index) {
                if ($scope.currentCatalog.data.$list[index].id) {
                    $scope.currentCatalog.data.$list[index].isEditing = false;
                    $scope.currentCatalog.data.$list[index].value = $scope.currentCatalog.data.$list[index].originalValue;
                } else {
                    $scope.currentCatalog.data.$list.splice(index, 1);
                }
            }

            $scope.removeField = function (id, index) {
                if (!$scope.currentCatalog.data.$meta) {
                    $scope.currentCatalog.data.$meta = {};
                }
                if (!$scope.currentCatalog.data.$meta.hasOwnProperty(id)) {
                    $scope.currentCatalog.data.$meta[id] = {};
                }
                $scope.currentCatalog.data.$meta[id].$deleted = true;
            };

            $scope.undoRemove = function (id) {
                delete $scope.currentCatalog.data.$meta[id];
            }

            $scope.save = function (editedCatalogInfo) {
                var payload = {
                    processId: $rootScope.selectedProcess.key
                };
                if (editedCatalogInfo) {
                    payload.reader = editedCatalogInfo.reader;
                    payload.writer = editedCatalogInfo.writer;
                    payload.name = editedCatalogInfo.name;

                    if (editedCatalogInfo.id) {
                        payload.id = editedCatalogInfo.id;
                    } else {
                        payload.data = {$list: []};
                        $http.post('/camunda/catalogs/api/create/', payload).then(function (result) {
                            if (result.data) {
                                toasty.success({title: "Info", msg: "Catalog saved!"});
                                fetchCatalogList(result.data);
                            } else {
                ...
                if ($scope.currentCatalog.data) {
                    for (var i = 0; i < $scope.currentCatalog.data.$list.length; i++) {
                        var checkingValue = $scope.currentCatalog.data.$list[i];
                        if (!checkingValue.value || "" === checkingValue.value) {
                            toasty.error({title: "Error", msg: 'Remove empty value(s)!'});
                            return;
                        }
                        if (_.some($scope.currentCatalog.data.$list, function (l, index) {
                            return index !== i && checkingValue.value === l.value;
                        })) {
                            toasty.error({
                                title: "Error",
                                msg: 'The value "' + checkingValue.value + '" has dublicates!'
                            });
                            return;
                        }
                    }
                    ...
                    for (var i = 0; i < $scope.currentCatalog.data.$list.length; i++) {
                        if ($scope.currentCatalog.data.$list[i].isEditing) {
                            delete $scope.currentCatalog.data.$list[i].isEditing;
                        }
                        if ($scope.currentCatalog.data.$list[i].originalValue) {
                            delete $scope.currentCatalog.data.$list[i].originalValue;
                        }
                    }
                }
                var json = JSON.stringify($scope.currentCatalog.data, function (key, value) {
                    if (key === "$$hashKey") {
                        return undefined;
                    }
                    return value;
                ...
                if (!payload.reader) {
                    payload.reader = $scope.currentCatalog.readerGroupIds[0];
                }
                if (!payload.writer) {
                    payload.writer = $scope.currentCatalog.writerGroupIds[0];
                }
                if (!payload.name) {
                    payload.name = $scope.currentCatalog.name;
                }
                if (!payload.id) {
                    payload.id = $scope.currentCatalog.id;
                }
                payload.data = json;

                console.log(payload);

                $http.put('/camunda/catalogs/api/update/', payload).then(function (result) {
                    if (result.data) {
                        toasty.success({title: "Info", msg: "Catalog saved!"});
                        fetchData();
                    } else {
                        toasty.error({title: "Error", msg: "Something went wrong..."});
                    }
                }).catch(function (error) {
                    toasty.error({title: "Error", msg: error});
                });
            ...
            $scope.FilterBySearch = function (item) {
                if (!$scope.searchText) {
                    ...
                    return item.name && item.name.toLowerCase().includes($scope.searchText.toLowerCase());
                }
            };
